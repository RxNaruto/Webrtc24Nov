...
export const Receiver = () => {
    ...
    const handleTranscription = (text: string, isFinal: boolean) => {
        ...
        if (isFinal) {
            if (socket) {
                socket.send(JSON.stringify({ type: 'transcription', text, isFinal: true, target: 'sender' }));
            }
    ...
    async function startSendingVideo() {
        ...
        try {
            ...
            pc.onicecandidate = (e) => {
                if (e.candidate) socket.send(JSON.stringify({ type:'iceCandidate', candidate:e.candidate, target:'sender' }));
            };
            ...
            socket.onmessage = async (event) => {
                ...
                if (data.type === 'createOffer') {
                    ...
                    socket.send(JSON.stringify({ type:'createAnswer', sdp: pc.localDescription, target:'sender' }));
            ...
            socket.send(JSON.stringify({ type:'createOffer', sdp: pc.localDescription, target:'sender' }));
        } catch (err) {
            ...
            {!isCallStarted ? (
                <div className="flex items-center justify-center min-h-[80vh]">
                    <div className="text-center max-w-md">
                        ...
                        <p className="text-slate-300 mb-8">Click the button below to join the video call and connect with the host.</p>
                        <button onClick={startSendingVideo} className="bg-green-600 hover:bg-green-500 text-white px-8 py-4 rounded-xl font-semibold">Join Video Call</button>
                    </div>
                </div>
            ) : (
                    ...
                    <div className="lg:col-span-2 flex flex-col space-y-4">
                        ...
                        {isCallStarted && (
                            <div className="flex-1 min-h-0">
                                ...
                                {isSpeechEnabled && <TranscriptionPanel transcriptions={transcriptions} isListening={isListening} onToggleListening={toggleListening} />}
                            </div>
                        )}
                    </div>
                </div>
